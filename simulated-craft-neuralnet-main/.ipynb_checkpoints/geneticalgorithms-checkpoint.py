...
MAX_TIME = 15. #layer_sizes=[8,10,10,2] means genome_length is 200!!!
Genome = Tuple[int] #Specimen = List[Genome, int, float]
Population = List[Genome]

FitnessFunc = Callable[[Genome], int] #takes a genome and spits back it's fitness value
PopulateFunc = Callable[[], Population] #takes nothing and spits out new solutions
SelectionFunc = Callable[[Population, FitnessFunc], Tuple[Genome, Genome]] #takes a population and fitness funct, selects 2 solutions for parents
CrossoverFunc = Callable[[Genome, Genome], Tuple[Genome, Genome]] #CrossoverFunc Takes 2 genomes and returns 2 new genomes
MutationFunc = Callable[[Genome], Genome] #MutationFunc takes one genome and returns a modified one
...
def fitness(genome : Genome, sim_class, layer_sizes):
    ...
    if time_in + time_out<60.: #if it takes longer than 60 seconds it is culled
...
def run_simulation(genome, sim_class, layer_sizes):
    # Create a ship object starting in center of display
    ...
    #sys.stdout.flush()
    return (ship.goal_counter, ship.time_in, ship.time_out, ship.closest_to_goal) #return results
